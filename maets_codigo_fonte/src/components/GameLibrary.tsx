    ...
    <div className="bg-steam-blue p-4 rounded">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
          <button 
            className={`${activeTab === 'all' ? 'bg-steam-accent' : 'bg-steam-blue-light'}`}
            onClick={() => setActiveTab('all')}
          >
            Todos os Jogos
          </button>
          <button 
            className={`${activeTab === 'recent' ? 'bg-steam-accent' : 'bg-steam-blue-light'}`}
      ...
      <div className="grid grid-cols-4 gap-4">
        {(activeTab === 'all' ? games : recentGames).map(game => (
          <div key={game.id} className="bg-steam-blue-light rounded overflow-hidden card">
            <div className="h-40 bg-steam-dark flex items-center justify-center">
              <img src={game.image} alt={game.title} className="w-full h-full object-cover" />
            </div>
            <div className="p-2">
              <h3 className="text-white">{game.title}</h3>
              <div className="flex justify-between text-steam">
                <span>{game.platform}</span>
                <span>{game.hoursPlayed}h</span>
              </div>
              <div className="text-steam text-sm">
